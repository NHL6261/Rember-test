<template>
  <div class="star star-24">
    <span class="star-item" :class="item" v-for="(item, index) in starClases" :key="index" ></span>
  </div>
</template>
...
export default {
    ...
    computed:{
        starClases(){
            ...
            // 全星
            let onClases =  Math.floor(rating)

            for(var i = 0; i < onClases ; i++){
                result.push('on')
            }
            // 半星
             (rating - onClases > 0) && result.push('half')
            // 没星
